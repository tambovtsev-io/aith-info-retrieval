...
class TfIdfRetriever:
    def __init__(
        ...
        self.vectorizer = TfidfVectorizer(analyzer=self.tokenizer)
        self._fit(self.documents)
    ...
    def retrieve(
        ...
        similarities = cosine_similarity(query_vector, self.document_vectors).ravel()
        top_indices = similarities.argsort()[-min(k, len(self.doc_ids)) :][::-1]
        return RetrievalResult(
            query_id=query_id,
            query=query,
            retrieved_docs=[
...
tok_simple = secuential_transforms([remove_punctuation, remove_stopwords])
tok_lemmatize = secuential_transforms([remove_punctuation, remove_stopwords, lemmatize])
tok_stem = secuential_transforms([remove_punctuation, remove_stopwords, stem])
...
def _build_measures(metrics: tp.Iterable[tp.Any], ks: tp.Iterable[int]) -> list[tp.Any]:
    ...
    for metric in metrics:
        ...
        if "@" in metric_name or metric_name in {"AP", "MAP"}:
            candidates = [metric]
        else:
            try:
                candidates = [metric @ k for k in unique_ks]
            except Exception:
                candidates = [metric]
